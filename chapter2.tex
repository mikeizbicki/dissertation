...
\fixme{We defer the discussion to Sections \ref{}.}
...
%The \defn{naive bayes classifier} is old and simple.
%It was widely used before the 2000s,
%but \citet{ng2002discriminative} showed that naive bayes works well only on very small data sets.
%Logistic regression is a better choice for large data sets.
...
%Estimators with closed form solutions (and hence exact merge functions) are rare in practice.
...
The simplest and most popular mergeable estimator is the \defn{averaging estimator}
\begin{equation}
...
\citet{zhang2012communication} provide a more general analysis showing that the \defn{mean squared error} (MSE) $\E\ltwo{\wstar-\wave}{}^2$ decays as $O((mn)^{-1} + n^{-2})$.
...
\citet{scott2016bayes} introduced the simplest merge procedure in their \defn{consensus monte carlo} (CMC) algorithm.%
\footnote{
...
The second category of samplers is based on \defn{subposterior distributions}.
...
\citet{minsker2014scalable} propose the \defn{median of subset posterior measures} method.
...
\begin{equation}
   % 
\end{equation}
\citet{srivastava2015wasp} propose the \defn{waserstein posterior} (WASP).
