...
%We are given a set of $mn$ data points $Z$ a set of models $\set H$
%A machine learning model consists of a set of parameters $\W$ and a loss function 
%This chapter introduces the semigroup framework for machine learning and describes

%In the most general machine learning setting,
%we are given a random variable $\Z$ and our goal is to esti
%a machine learning problem is defined by a set $\Z$ of data points,
%a set $\W$ of parameters,
%and an algorithm $A : \Z \to \W$
...
and $f$ be a function from $\Z$ to a space $\W$ called the \defn{parameter space}.
Our goal is to estimate $\E_{\z\in\Z} f(\z)$ based on a sample of points $Z$ from $\set Z$.
We call a function $\hat f : \{\Z\} \to \W$ an \defn{estimator} of $f$ if
\begin{equation}
...
\begin{equation}
    \ltwo{\wstar-\what}
\end{equation}
We define \defn{true loss} to be
\begin{equation}
    \riskstar = \E_{\z\in\Z}\Loss(\z;\wstar)
\end{equation}
and the \defn{empirical loss} of an estimator $f$ to be 
\begin{equation}
...
%Let $\z$ be a random variable drawn from some probability space $\Z$,
%and $f : \z \to \W$ be a function.
%
...
\footnote{
    A \defn{tolerance relation} is an equivalence relation that need not obey transitivity.
...
\footnote{
    A set of points is called an \defn{algebraic variety} if it is the set of zeros of a polynomial equation.
}
...
In the popular but naive \defn{grid search} technique,
...
%The empirical loss calculated from a data set is not a good measu
%Cross validation is a method for measuring the quality of an estimator.

The \defn{bootstrap} improves the statistical properties of the validation set estimator but is more expensive computationally.
...
\defn{Cross validation} also improves on the validation set method.
...
Naive $k$-fold cross validation is an expensive procedure taking time $O(k f(n))$.
For many models, fast cross validation procedures have been developed.
...
All of these methods fit the semigroup framework,
and so these fast cross validation methods can be though of as special cases of our algorithm.
\citet{joulani2015fast} provides a tree based fast cross validation algorithm for incremental learning algorithms.
All previous work on fast cross validation methods has been for the single machine case,
whereas we provide a method for the distributed learning case.
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \For {$i = 1$ \TO $k$}
            ...
            \State $\wprefix_{,i} = \merge_f(\wprefix_{,i-1}, \what_i)$ 
        \EndFor
        %\vspace{0.1in}
        \State // calculate suffixes
        \For {$i = k$ \TO $1$}
            %\State $\wsuffix_{,i} = \wsuffix_{,i+1} \mappend{} \what_i$ 
            \State $\wsuffix_{,i} = \merge_f(\wsuffix_{,i+1} , \what_i)$ 
        \EndFor
        %\vspace{0.1in}
        \State // merge models and calculate estimated risk
        \For {$i = 1$ \TO $k$}
            %\State $\what_{-i} = \wprefix_i \mappend{} \wsuffix_i$
            \State $\what_{-i} = \merge_f(\wprefix_i , \wsuffix_i)$
            \State $\riskemp_i = \Loss(\what_{-i},Z_i)$
        \EndFor
        %\vspace{0.1in}
        \State \Return $\frac 1 k \sum_{i=1}^k \riskemp_i$
    \end{algorithmic}
