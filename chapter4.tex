...
This chapter provides both practical and theoretical improvements to the cover tree.
...
Section \ref{} formally introduces metric spaces and their importance in machine learning.
Section \ref{} 
Section \ref{} reviews the existing techniques for faster nearest neighbor queries.
Section \ref{} formally describes the original cover tree data structure.
Section \ref{} presents the simplified cover tree heuristic.
Section \ref{} concludes the chapter with experiments demonstrating the improvements success in practice.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Metric spaces and nearest neighbors in machine learning}

Metric spaces are a generalization of Euclidean spaces that are often more natural to work with.
A set $\set X$ equipped with a distance function $\distf : \set X \times \set X \to \R$ is a \emph{metric space} if it obeys the following three properties:
\begin{enumerate}
...
\end{enumerate}
In this chapter, we will use the script $\set X$ notation for metric spaces that may be either infinite or finite,
and the 
...
Then the \emph{nearest neighbor} $y^*\in X$ is defined as $y^*=\argmin_{y\in X} \dist{x}{y}$.
An \emph{$\varepsilon$-approximate nearest neighbor} (for $\varepsilon\ge1$) is a point $\hat y$ satisfying $\dist{x}{\hat y} \le \varepsilon \cdot\dist{x}{y^*}$.

\subsection{Metric learning}

\cite{kulis2013metric} and \cite{bellet2013survey} provide tutorials.

\subsection{Infinite metrics}
...
but Section \ref{sec:original} improves these bounds to use the doubling dimension.
...
A \emph{$\delta$-packing} of a metric space $\set X$ is a set $\{x_1,x_2,...,x_M\} \subseteq \set X$ such that $\dist{x_i}{x_j} > \delta$ for all $i \ne j$.
...
Sets with small aspect ratio are called \emph{fat},
and sets with large aspect ratio are called \emph{skinny}.
...
{\bfseries function} \rebalanceHelper(cover trees $p$ and $q$, point $x$)
...
Previous work on parallelizing cover trees applied only to the GPU \cite{Kumar2010}.
...
{\bfseries function} \mkprocedure{merge}(cover tree $p$, cover tree $q$)
...
{\bfseries function} \mkprocedure{mergeHelper}(cover tree $p$, cover tree $q$)

%\emph{prerequisite:} $\level{p}=\level{q}$

%\emph{prerequisite:} $d(p,q) \le \covdist p$
\emph{prereqs:} $\level{p}=\level{q}$, $d(p,q)\le\covdist p$
