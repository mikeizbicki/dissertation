...
%On the theoretical side, this chapter improves the analysis of the cover tree's runtimes.
...
%The following example shows that the expansion dimension

\begin{example}
    \label{ex:cexp:1}
    A subset of a metric space may have arbitrarily large expansion dimension compared to the host space.
    Let $x_i = 2^{-i}$ for all $i\in\mathbb N$,
    and $X_n = \{x_0,x_1,...,x_n\}$.
    $X_n$ is a subset of $\mathbb R$, which has expansion number $O(1)$,
    but $X_n$ has expansion number $O(n)$.
\end{example}

\begin{example}
    \label{ex:cexp:2}
    Adding a single point to a space may change the expansion dimension by an arbitrary amount.
    Let $X$ be a finite metric space with $n$ elements and expansion constant $\cexp\ltlt n$,
    and let $\diam{X}$ be the maximum distance between any two points in $X$.
    Construct a new space $Y=X\cup\{y\}$ where $y$ is a point not in $X$.
    Define the distance between $y$ and any point in $X$ to be $\diam{X}$.
    This distance satisfies the triangle inequality,
    and so $Y$ is a metric space.
    The expansion dimension of $Y$ is $n$.
\end{example}

The doubling dimension discussed next does not suffer from either of the defects in Examples \ref{ex:cexp:1} or \ref{ex:cexp:2}.
Unfortunately, it cannot be used to measure the difficulty of exact nearest neighbor queries.
The hole dimension (introduced in two sections) sits ``in between'' the doubling dimension and the expansion dimension.
It does not suffer from the defect of Example \ref{ex:cexp:1},
but it does suffer from the defect of Example \ref{ex:cexp:2}.
Unlike the doubling dimension, however, the hole dimension is suitable for measuring the difficulty of exact nearest neighbor queries.
...
\begin{proof}
    \fixme{Several papers state this without proof or reference.  I need to find the original source.}
    %Let $P$ be a minimal cover of $\set X$.
    %If $P\subseteq X$, then \eqref{eq:cdoubinclusion} holds.
    %Otherwise, there must exist a point $x$ in $P$ not in $X$.
    %We show that removing a single point from $\set X$ cannot increase the doubling number.
    %The inequality \eqref{eq:cdoubinclusion} then follows by induction.
    %Let $P$ be a minimal covering of $\set X$, and $x$ be an arbitrary point in $\set X$.
    %If $x\not\in P$, then $P$ is a covering of $\set X$ and so $\cdoub(\set X-\{x\})\le\cdoub(\set X)$.
...
The idea is to create a dimension that is suitable for measuring the difficulty of exact nearest neighbor queries while being more robust than the expansion dimension.
To do this, we modify the definition of the doubling dimension so that we ignore ``holes'' around each point in the space.
In particular, the \defn{hole number} is defined as
\begin{align}
    \label{eq:chole}
    \chole
    = 
    ...
    .
\end{align}
The \defn{hole dimension} is then
\begin{align}
    \holedim = \log_2 \chole
    .
\end{align}
...
\begin{proof}
    ...
    More generally,
    when the value of $\epsilon$ in \eqref{eq:chole} is small,
    then the hole dimension is close to the doubling dimension.
    When the value of $\epsilon$ is large, 
    then the hole dimension is larger than the doubling dimension.
\end{proof}
...
\begin{lemma}
    Let $\set X=\R^d$ with the standard $L_2$ distance function.
    Then $\holedim\set X = \Theta(d)$.
\end{lemma}
\begin{proof}
    For all points $x\in\set X$,
    the value of $\epsilon$ in \eqref{eq:chole} is 0.
    Therefore the hole dimension is equivalent to the doubling dimension.
    The result follows by Lemma \ref{lemma:doubleEuclidean},
    which bounds the doubling dimension of Euclidean space.
\end{proof}
...
\begin{example}
    \label{ex:chole:1}
    The hole dimension of a metric space may be arbitrarily smaller than the expansion dimension of the space.
    We show this using the same metric space as from Example \ref{ex:cexp:1}.
    Let $x_i = 2^{-i}$ for all $i\in\mathbb N$,
    and $X_n = \{x_0,x_1,...,x_n\}$.
    Then $X_n$ has expansion number $O(n)$ and hole dimension $O(1)$.
\end{example}

\noindent
In Section \ref{sec:original},
we provide novel runtime bounds for the cover tree based on the hole dimension.
We show that the runtime in terms of the hole dimension is significantly smaller than the runtime in terms of the expansion dimension ($O(\chole^4)$ vs $O(\cexp^{12}$).
Combined with the fact that $\chole$ can be much smaller than $\cexp$,
this implies a bound that is potentially much stronger for certain datasets.
...
For example,
cover trees have been used to speed up
support vector machines \citep{Segata2010},
dimensionality reduction \citep{Lisitsyn2013},
gaussian processes \citep{moore2014fast}, 
and reinforcement learning \citep{Tziortziotis2014}.
%
%\fixme{add more examples of cover tree uses}
This widespread adoption is likely due to the fast, 
easy to use reference implementation provided by \citet{beygelzimer2006cover}.
...
\begin{theorem}
    ...
    %Let $\varepsilon\ge2$.
    %Then $\findnnorig(\{p\},x)$ has runtime $O(\cdoub^4\log\aspect{})$.
    %If the data is generated i.i.d.\ from a well behaved distribution,
    %then the runtime is $O(\cdoub^4\log n)$.
    %If $\varepsilon > 0$,
...
\begin{theorem}
    ...
    Let $\varepsilon\ge1$.
    Then $\findnn(\{p\},x,\varepsilon)$ has runtime $O(\cdoub^5\log\aspect{})$.
    %If the data is generated i.i.d.\ from a well behaved distribution,
    %then the runtime is $O(\cdoub^4\log n)$.
    %If $\varepsilon > 0$,
...
%In our tests, the garbage collector typically required between 5 to 10 percent of our program's runtime.
...
%It is impossible to guess how persistence might affect the runtime of our cover tree.
...
\caption{
    ...
    Reducing the number of cache misses results in fewer stalled cycles, and so faster runtimes.
