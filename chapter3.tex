...
%the quantity $\ltwo{\what-\werm}{}^2$ decays as $\Omega(\gamma^2 \I^{-1}/n^2)$.
...
\begin{equation}
\label{eq:werm}
%\werm=\argmax_\w \sum_{i=1}^{mn} g(y_i-\trans\x_i\w)
...
%The idea of our proposed solution is to find the best linear combination of the $\werm_i$s using a second round of optimization.
...
\node at (3.1,1.85) {$\werm_1$};
\node[dot] (wave) at (4.75,1.10) {};
\node at (4.9,1.55) {$\werm_2$};
%\node[dot] (wave) at (3.8,1.25) {};
...
%We show in our experiments that when the problem is large enough to take a full day to solve for the $\werm_i$s on the first round, the second round takes only several minutes.
...
%This regularization is in principle no more difficult to optimize than the term $\reg(\w)$ from Equation $\ref{eq:werm}$;
...
%Throughout this section, we will assume that the $\werm_i$s, $\vowa$, and $\vfull$ estimators all satisfy the SGT condition.
...
%\begin{theorem}
%\label{theorem:wowa}
%Assume the Hessian Condition and that both the $\werm_i$s and $\vowa$ satisfy the SGT condition.
%Let $\nowa=mn/d$ and $t>0$.
...
\addplot[werm,no marks] table [x index=0,y index=5] {#1};
\addplot[wermi,no marks] table [x index=0,y index=7] {#1};
...
&\hspace{-0.5cm}\mklambdaplot
    ...
    {,ymin=10^0,ymax=10^2}
    %{ \node at (2,2.8) {\tiny\textcolor{wermi}{$\werm_i$}};
      %\draw[->,wermi] (1.8,2.95) -- (1.8,3.3);
    {}
    {15}
&\hspace{-0.5cm}\mklambdaplot
...
\caption{
    ...
    %In particular, it scales with $m$ at the same rate as $\werm$, whereas $\wave$ and $\wboot$ do not scale well with $m$ on this synthetic data.
...
%Surprisingly, $\wowa$ even outperforms the oracle $\werm$ in some regimes.
...
\addplot[werm,no marks] table [x index=3,y index=5] {#1};
\addplot[wermi,no marks] table [x index=3,y index=7] {#1};
...
\caption{
    ...
    %Surprisingly, additional regularization introduced by OWA lets it outperform the oracle estimator $\werm$ in some cases.
...
%why does $\wowa$ behave better than $\werm$ in some circumstances?
