...
%Typically, the input set will contain only a single cover tree, 
%but the algorithm is written more generally due to its recursive nature.
...
%\begin{pseudocode}{$\findnn$(set of cover trees $P$, query  point $x$, nearest neighbor so far $y$, tolerance $\epsilon$)}
%\If {$P = \{\}$}
    %\State\Return $y$
%\Else
    %\For {$p\in P$}
    ...
    %%\For {$p\in P$}
        ...
        %%\EndFor
    %%\EndFor
    %\State let $P' = \{ q : q\in\children{p}, p\in P, \dist{x}{q} \le \dist{x}{y}/\epsilon+\maxdist{q}\}$
...
$\ctinsert$ is a simple wrapper around $\ctinsertloop$ that ensures all $x$ can be inserted into the tree $p$.
If $x$ is too far away to be a child, 
then a new tree is created with $x$ as the root and $p$ as a child.
The level of $x$ is calculated to exactly satisfy the covering invariant.
Otherwise, if $x$ can be inserted as a child of $p$ without breaking the separating invariant,
it is.
Otherwise, $x$ must satisfy the preconditions of $\ctinsertloop$,
...
\begin{algorithm}
    ...
    \Else
        %\State \Return node(data=data($p$),level=$\level p$,children=$\ctinsertloop(\children{p},x)$)
        %\State \Return the single tree in the set returned by $\ctinsertloop(\{p\},x)$
        \State $\ctinsertloop(\{p\},x)$
        \label{ctinsert:line:ctinsertloop}
        %\State let $P = \ctinsertloop(\children{p}, x)$
        %\State let $p'$ be the node $p$ with its children set to $P$
        %\State \Return $p'$
    \EndIf
\end{algorithmic}
\end{algorithm}
...
The idea is to perform a breadth first traversal of the tree,
but at each level we descend only a small subset of the nodes.
In order to maintain the global separating invariant, 
several subtrees must be traversed simultaneously.
This complicates the pseudocode.

%$\ctinsert$ is a simple wrapper around the more complex helper function $\ctinsertloop$,
%which handles the recursive search.
$\ctinsert$ has three cases.
In the first case, it checks to see whether $x$ is too far away from $p$ to be inserted as a child.
If so, it creates a new tree rooted at $x$,
sets $p$ as its child,
and sets the level to satisfy the covering invariant.
In the second case, it checks whether $x$ can be inserted as a child into $p$ while maintaining the separating invariant.
The final case calls $\ctinsertloop$ to perform the recursive traversal.
...
\end{pseudocode}
\noindent
$\ctinsertloop$ performs a recursive traversal of the input set $P$.
Semantically, $P$ is the set of subtrees that we must simultaneously traverse.
$\ctinsertloop$ returns a new set $P'$ equal to $P$ except that one subtree in $P'$ has been modified to contain $x$.
For example, when $\ctinsertloop(\{p\},x)$ is called in line \ref{ctinsert:line:ctinsertloop} above,
we are guaranteed to return a set containing a single tree rooted at $\datapoint{p}$.
%This property ensures that the node created on line 4 of $\ctinsert$ will obey the separating invariant.
...
\noindent
There are two cases.
In the first case, $x$ can be inserted directly as a child of some $\hat p\in P$ without violating the separating invariant.
\begin{pseudocodeM}
    \If {$\dist{\hat q}{x} \ge \covdist{\hat q}$}
        \State let $\hat p = \argmin_{p\in P} \dist{p}{x}$
        \State let $\hat p' = \hat p$ with $x$ added as a child
...
\end{pseudocodeM}
\noindent
In the second case, $x$ cannot be inserted as a child because this would violate the global separating invariant.
So we create a subset of $Q$ that might contain $x$, and insert $x$ into that subset.
\begin{pseudocodeB}
    \Else
        \State let $Q_x = \{q : q\in Q, \dist{q}{x} \le \covdist{q}\}$
...
\begin{pseudocode}{$\dualnn$(set of cover trees $P$, set of cover trees $R$, tolerance $\epsilon$)}
    \If {$\level P \ge \level R$}
        %\State $P' \leftarrow \{\}$
        \For {$p\in P$}
            \State $\dualnn(\children{p},R,\epsilon)$
            %\If {$\level p = \level P$}
        ...
        \For {$r\in R$}
            %\If {$\level r = \level R$}
                ...
                \For {$p\in P$}
                    ...
                    \EndIf
                    \If {$\maxdist{p} \le \dist{r}{p}/\epsilon+\maxdist{r}$}
...
\begin{algorithm}[H]
    ...
    \vspace{0.1in}
    {\bfseries function} \dualnn(set of cover trees $P$, set of cover trees $R$, tolerance $\epsilon$)
...
\begin{theorem}
    ...
    After running $\dualnn(\{p^*\},\{r^*\},\epsilon)$,
    $nn[q]$ contains an $\epsilon$-approximate nearest neighbor of $q$ in $r$ for all $q$ in $p$.
\end{theorem}

\begin{proof}
    We will show that for every node $p$ in $p^*$,
    $\dualnn$ will be called with $p\in P$ and some $\epsilon$-ann in $R$.
    When this happens, the if statement on line 8 will ensure that $nn[p]$ gets updated to a $\epsilon$-ann.
\end{proof}

\begin{theorem}
    Let $p$ and $r$ be the root of two cover trees.
    The runtime of $\dualnn(\{p\},\{r\},\epsilon)$ for all $\epsilon\ge2$ is $O(\doubnum^4\log\aspect{})$.
\end{theorem}
\begin{proof}
\end{proof}
