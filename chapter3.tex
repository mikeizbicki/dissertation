...
\newcommand{\holenum}{c_\textnormal{hole}}
...
\begin{proof}
    ...
    By induction, we assume that $y^*$ is in $\data{P}$ and hence in $\data{Q}$.
    ...
    \begin{equation}
        ...
        .
    \end{equation}
    So $q^*$ is always included in the set $Q_x$,
...
\begin{algorithmic}[1]
    ...
    \Else
    \State let $Q_x = \{ q : q\in Q, \dist{x}{q} \le \hl{$\dist{x}{\hat q}/(1+\varepsilon)$}+\maxdist{q}\} \hl{$\cup \{\hat q\}$}$
...
\begin{theorem}
    $\findnn(\{p\},x,1)$ has runtime $O(\holenum^4\log\aspect{})$.
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    By induction, we assume that $y^*_s$ is in $\data{P}$ and hence in $\data{Q}$.
