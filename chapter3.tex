...
\begin{algorithm}[H]
    ...
    \vspace{0.1in}
    {\bfseries function} \findnn(set of cover trees $P$, query  point $x$, nearest neighbor so far $y$)
...
\begin{algorithmic}[1]
    ...
    \Else
        \For {$p\in P$}
            ...
            \EndIf
        \EndFor
        \State let $P' = \{ q : q\in\children{p}, p\in P, \dist{x}{y} > \dist{x}{q} - \maxdist{q} \}$
        \State\Return $\findnn(P',x,y)$
    \EndIf
\end{algorithmic}
\end{algorithm}
